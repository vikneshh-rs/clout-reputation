...
export default function RepRecovery(props: any) {
  ...
  const isAllowed = user && user.role === 'REP';
  if (!isAllowed) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans">
        <AlertCircle className="h-12 w-12 text-slate-800 mb-4" />
        <h1 className="text-xl font-bold text-slate-900">Access Denied</h1>
        <p className="text-slate-500 text-sm mt-1">Representative permissions required.</p>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout title="My Onboarded Customer Recovery Queue" theme={theme} toggleTheme={toggleTheme}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Assigned Customer Recovery (Read-Only)</h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Monitor experience recovery tickets for businesses you onboarded.
          </p>
        </div>

        {/* Rep onboarded Business Selector */}
        <div className="flex items-center gap-2 bg-white/80 backdrop-blur-md border border-slate-100 px-3 py-1.5 rounded-2xl shadow-sm max-w-xs w-full">
          ...
          {loadingBiz ? (
            ...
            >
              <option value="ALL">All My Onboarded Businesses</option>
              {businesses.map((b) => (
      ...
      <CustomerRecoveryModule businessId={selectedBizId === 'ALL' ? undefined : selectedBizId} readOnly={true} />
    </DashboardLayout>
  );
}
