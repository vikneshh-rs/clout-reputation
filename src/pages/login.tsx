...
import { Lock, AlertCircle, Loader2, ArrowLeft, ShieldCheck, Briefcase } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await login(businessName, password, 'business');
    setLoading(false);

    if (!res.success) {
      setError(res.error || 'Invalid business name or password');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative font-sans transition-colors duration-200">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        <div className="w-full flex justify-start px-4 sm:px-0">
          <Link href="/" className="inline-flex items-center text-xs font-semibold text-slate-500 hover:text-slate-909 mb-6 transition-colors">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
        <div className="bg-white/80 backdrop-blur-md py-8 px-4 sm:rounded-2xl sm:px-10 border border-slate-200/50 transition-colors">
          {error && (
          ...
          {successMsg && (
            <div className="mb-4 p-3 rounded-xl bg-emerald-50 border border-emerald-200/50 text-emerald-700 text-xs flex items-start">
              <ShieldCheck className="mr-2 h-4 w-4 flex-shrink-0 mt-0.5" />
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="businessName" className="block text-xs font-semibold text-slate-655 uppercase tracking-wider">
                Business Name
              </label>
            ...
            <div>
              <div className="flex items-center justify-between">
                <label htmlFor="password" className="block text-xs font-semibold text-slate-655 uppercase tracking-wider">
                  Password
                </label>
              </div>
              <div className="mt-1.5 relative rounded-md shadow-sm">
